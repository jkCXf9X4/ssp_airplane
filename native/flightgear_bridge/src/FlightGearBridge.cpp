...
namespace
{
  ...
  const Aircraft_StringFieldBinding *find_string_binding(fmi2ValueReference value_reference)
  {
    for (size_t i = 0; i < Aircraft_FlightGearBridge_StringBindingCount; ++i)
    {
      const auto &binding = Aircraft_FlightGearBridge_StringBindings[i];
      if (binding.value_reference == static_cast<int>(value_reference))
      {
        return &binding;
      }
    }
...
extern "C"
{
  ...
  fmi2Status fmi2GetString(fmi2Component component, const fmi2ValueReference vr[], size_t nvr, fmi2String value[])
  {
    ...
    for (size_t i = 0; i < nvr; ++i)
    {
      const auto *binding = find_string_binding(vr[i]);
      if (binding == nullptr)
      {
        return fmi2Error;
      }
      value[i] = binding->get(instance).c_str();
    }
    return fmi2OK;
  }
  ...
  fmi2Status fmi2SetInteger(fmi2Component component, const fmi2ValueReference vr[], size_t nvr, const fmi2Integer value[])
  {
    ...
    for (size_t i = 0; i < nvr; ++i)
    {
      ...
      *binding_ptr<int>(instance, *binding) = value[i];
      network_param_changed = network_param_changed || vr[i] == AIRCRAFT_FLIGHTGEARBRIDGE_VR_TELEMETRY_PORT || vr[i] == AIRCRAFT_FLIGHTGEARBRIDGE_VR_CONTROL_PORT;
    }
  ...
  fmi2Status fmi2SetString(fmi2Component component, const fmi2ValueReference vr[], size_t nvr, const fmi2String value[])
  {
    ...
    for (size_t i = 0; i < nvr; ++i)
    {
      ...
      binding->get_mut(instance) = value[i] ? value[i] : "";
