...
The following mission-driven use cases connect the SysML requirements to concrete simulation scenarios stored in `resources/scenarios/`. Each use case is backed by deterministic waypoint files so simulation runs and tests can validate requirement coverage.
...
These curated scenarios are referenced by the simulation tests so regressions that break requirement coverage are caught early. Each scenario simulation produces a CSV and a requirements-oriented summary JSON; see `docs/results_and_evaluation.md` for how to interpret the outputs.
