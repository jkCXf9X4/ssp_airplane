...
model AutopilotModule
  ...
  parameter Real altitudeToPitchGain  = 10       "deg/km, maps altitude error to desired pitch";
  parameter Real pitchAngleGain       = 1 / 20.0 "deg^-1, maps pitch error to stick";
  parameter Real maxPitchCommand_deg  = 20       "Limit for commanded pitch angle";
...
protected
  ...
  Real horizontalDistance_km;
  Real altitudeSeparation_km;
  ...
  // Cascaded altitude / pitch control
  Real targetPitch_deg;
  Real pitchError_deg;
...
equation
  ...
  horizontalDistance_km = if waypointCount < 1 then 0 else sqrt(dx_km ^ 2 + dy_km ^ 2);
  altitudeSeparation_km = abs(dz_km);
  distanceToWaypoint_km = if waypointCount < 1 then 0 else sqrt(dx_km ^ 2 + dy_km ^ 2 + dz_km ^ 2);

  // Heading to waypoint (deg)
  targetHeading = if waypointCount < 1 then
                    currentOrientation.yaw_deg
                  else
                    Math.atan2(dy_km, dx_km) * radToDeg;
  ...
  headingError = Math.atan2(
                   ...
                   Math.cos((targetHeading - currentOrientation.yaw_deg) * degToRad)
                 ) * radToDeg;
  ...
  rollCmd_raw = noEvent(
                  max(-1.0,
                      min(1.0, headingGain * headingError)));

  // ---- Cascaded altitude → pitch control ----
  // Outer loop: altitude error -> desired pitch angle (deg), with limits
  targetPitch_deg = noEvent(
                      max(-maxPitchCommand_deg,
                          min( maxPitchCommand_deg,
                               altitudeToPitchGain * altitudeError)));

  // Inner loop: pitch error -> stick command
  pitchError_deg = targetPitch_deg - currentOrientation.pitch_deg;

  pitchCmd_raw = noEvent(
                   max(-1.0,
...
// Simple solution, should most likely switch to pid regulated, good enough for now
  ...
  arrived = waypointCount > 0 and
            horizontalDistance_km <= waypointProximity_km and
            altitudeSeparation_km <= waypointAltitudeProximity_km;
